...
def actualizar_modelo(it_file, vt_file, LOW_FACTOR):
    ...
    # ✅ convertir VUELO a texto (ERROR RESUELTO)
    it['VUELO'] = it['VUELO'].astype(str)
...
st.markdown("Sube itinerario y ventas para analizar el negocio")
...
if df is not None:
    ...
    with col1:
        ...
        fig1 = px.bar(ventas_destino, x="DESTINO", y="VENTAS", title="Ventas por destino")
        st.plotly_chart(fig1)

    with col2:
        efectividad_destino = df.groupby("DESTINO")['EFECTIVIDAD'].mean().reset_index()
        fig2 = px.bar(efectividad_destino, x="DESTINO", y="EFECTIVIDAD", title="Efectividad")
